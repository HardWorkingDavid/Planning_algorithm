...
int Trajectory_Smoothing::Write() {
    std::ifstream input_x("/home/bigdavid/视频/Planning_algorithm/src/QP_Smooth_referenceline/src/global_path_x.txt");
    if (!input_x.is_open()) {
    ...
    std::ifstream input_y("/home/bigdavid/视频/Planning_algorithm/src/QP_Smooth_referenceline/src/global_path_y.txt");
    if (!input_y.is_open()) {
        std::cout << "Failed to open the file." << std::endl;
        return 1;
    }
